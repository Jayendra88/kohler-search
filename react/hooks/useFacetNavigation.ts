...
const removeElementAtIndex = (strArray: string[], index: number): string[] =>
  strArray.filter((_, i: number) => i !== index)

const upsert = (array: any[], item: any): void => {
...
const removeMapForNewURLFormat = (
  ...
  return mapArray
    .filter((mapItem: string) => !mapsToFilter.includes(mapItem))
    .join(MAP_VALUES_SEP)
}

const getCleanUrlParams = (currentMap: string): URLSearchParams => {
 ...
 */
const findFacetIndex = (query: string[], map: string[], facet: any): number => {
  return zip(query, map).findIndex(([value, valueMap]: [string, string]) =>
    compareFacetWithQueryValues(value as string, valueMap as string, facet)
  )
}
